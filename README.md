<!-- Level 1: Simple bio and stats -->

Hello, I'm Austin.

I am an IAM Engineer with 5 years of enterprise experience designing and securing identity ecosystems across Azure AD / Entra ID, Okta Identity Cloud, SailPoint ISC, and Delinea Secret Server.

My work focuses on identity governance, privileged access security, and authentication hardening across hybrid and cloud environments.

I am currently expanding deeper into Azure cloud security architecture and identity-driven security controls as part of my long-term goal of becoming a Cloud Security Architect.

What I’m Currently Working On

I am actively building hands-on projects to deepen my understanding of SailPoint Identity Security Cloud (ISC) and identity governance automation.

Current focus areas include:

• SailPoint ISC connector configuration and aggregation logic
• Identity lifecycle automation (Joiner / Mover / Leaver)
• Access Profiles, Business Roles, and RBAC modeling
• Account correlation rules across authoritative sources
• Identity governance workflows and certification campaigns
• API integrations and automation using REST APIs and JSON

This repository documents the lab environments, configurations, and automation exercises I am completing as part of my SailPoint ISC training.

Technologies & Platforms

Identity Platforms

SailPoint ISC
Okta Identity Cloud
Azure AD / Entra ID

Cloud & Security

Conditional Access
Privileged Identity Management (PIM)
Azure RBAC
Zero Trust authentication models

Privileged Access Management

Delinea Secret Server
Azure Key Vault
AWS Secrets Manager

Automation & Protocols

PowerShell
REST APIs
JSON
Postman

Identity & Authentication

SAML 2.0
OAuth 2.0
OIDC

Directory & Hybrid Identity

Active Directory
Azure AD Connect
